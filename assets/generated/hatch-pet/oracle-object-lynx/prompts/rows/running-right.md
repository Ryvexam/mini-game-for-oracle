Create one horizontal animation strip for Codex pet `oracle-object-lynx`, state `running-right`.
...
Identity: same pet in every frame: Cute pixel-art lynx database guide with teal scarf and tiny database charm, consistent with Oracle Object Island game sprites.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: Use a crisp pixel-art game pet style, readable in 192x208 cells, no text or logos..
