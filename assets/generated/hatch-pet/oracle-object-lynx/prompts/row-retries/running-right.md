Create Codex pet row `running-right` for `oracle-object-lynx`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Cute pixel-art lynx database guide with teal scarf and tiny database charm, consistent with Oracle Object Island game sprites.. Preserve silhouette, face, palette, material, proportions, markings, and props.
